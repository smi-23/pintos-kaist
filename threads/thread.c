...
tid_t thread_create(const char *name, int priority, thread_func *function,
  ...
  thread_unblock(t);
  /*
   * 새로 생성한 priority와 현재 실행중인 priority를 비교해서 새로 생성한
   * priority가 더 크다면 yield해서 선점 unblock에서 ready_list에
   * insert_order하기 때문에 (!list_empty(&ready_list))예외 처리는 생략
   */
...
void set_priority(struct thread *target, int new_priority) {
  ...
  /* ready_list가 비어 있지 않고, 새로 생성한 priority와 현재 실행중인
   * priority를 비교해서 새로 생성한 priority가 더 크다면 yield해서 선점  */
  if (!list_empty(&ready_list) &&
      target->priority < get_thread_elem(list_front(&ready_list))->priority)
    thread_yield();
}

/**
 * @brief donation list가 비어있지 않으면 `max(donation_list)`를, 비어있다면
 * original priority
 */
 ...
 */
void thread_set_nice(int nice) { set_nice(thread_current(), nice); }

/* Returns the current thread's nice value. */
int thread_get_nice(void) { return get_nice(thread_current()); }
...
int thread_get_recent_cpu(void) { return get_recent_cpu(thread_current()); }
...
int get_priority(struct thread *target) {
  ...
  struct list_elem *max_elem =
      list_max(&target->donation_list, priority_asc_d, NULL);

  return get_priority(get_thread_d_elem(max_elem));
}

int get_nice(struct thread *target) { return target->nice; }

void set_nice(struct thread *target, int val) { target->nice = val; }
...
void update_recent_cpu(struct thread *target) {
  // fixed_point에 100을 곱한 뒤 int형으로 변환했기 때문에 이를 다시
  // fixed_point로 변환
  const fixed_point load_avg = g_load_avg;
  const fixed_point recent_cpu = target->recent_cpu;
  // `(2 * avg) / ((2 * avg) + 1)`
  const fixed_point decay_rate = FXP_DIV(
      (FXP_MUL_INT(load_avg, 2)), (FXP_ADD_INT(FXP_MUL_INT(load_avg, 2), 1)));

  // decay_rate * recent_cpu + nice
  target->recent_cpu =
      FXP_ADD_INT(FXP_MUL(decay_rate, recent_cpu), target->nice);
}
